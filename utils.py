        ...
        for x in range(0,rows):
            for y in range(0,columns):
                if img_array[x][y].shape[:2] == img_array[0][0].shape[:2]:
                    ...
                    img_array[x][y] = cv2.resize(imgArray[x][y], (img_array[0][0].shape[1], img_array[0][0].shape[0]), None, scale, scale)
        ...
        for x in range(0,rows):
            if img_array[x].shape[:2] == img_array[0].shape[:2]:
                img_array[x] = cv2.resize(img_array[x],(0,0),None,scale,scale)
                    ...
                    img_array[x] = cv2.resize(imgArray[x], (imgArray[0].shape[1], imgArray[0].shape[0]), None,scale, scale)
            if len(img_array[x].shape) == 2:
                img_array[x] = cv2.cvtColor(imgArray[x], cv2.COLOR_GRAY2BGR)
